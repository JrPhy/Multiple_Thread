...
```
#### 3. 比較與交換 CAS
在原子操作實作中，其中一個方式是比較並交換 Compare-and-Swap (CAS)，也就是先確認新值與舊值是否相同，如果相同就可以改變。
```C
...
```
不過即便新值與舊值相同，也不能保證在中間這段時間值沒有改變過。例如有三個執行緒 A, B, C 都對 x 做計算且時序如下
|x |Thread A | Thread B | Thread C |
|:--|:--|:--|:--|
|R | x |  |  | 
|M |  | x++ |  | 
|M |  |  | x-- | 
...
對於 A 來說可能 x 沒改變，但實際上已經被改過又改回來了。所以在簡單的資料結構中會加入版本號來去避免這件事發生，也就是同時檢查原本的值與版本號
```c
struct val{
    int a, ver;
}
